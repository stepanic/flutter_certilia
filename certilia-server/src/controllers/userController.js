...
export const getExtendedUserInfo = async (req, res, next) => {
  try {
    ...
    if (skipUserInfo) {
      // Use ID token claims directly in production
      source = 'id_token_claims';
      logger.info('Skipping userinfo endpoint in production, using ID token claims');
      ...
      if (certiliaTokens.id_token) {
        ...
        return res.status(400).json({
          ...
          message: 'ID token is required when userinfo endpoint is disabled'
        });
      }
    } else {
