...
Note: This data is NOT stored anywhere and will be lost when this dialog is closed.
  ...
  /// Perform stateless user info fetch using stored token
  Future<void> _performStatelessUserFetch(BuildContext context) async {
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: CertiliaTheme.spaceMD),
            Text(
              'Fetching user info with stored token...',
              style: CertiliaTextStyles.bodyMedium(isDark),
            ),
          ],
        ),
    ...
    try {
      // Try to get stored token from the wrapper's static method
      final storedToken = await CertiliaStatefulWrapper.getStoredAccessToken();
      ...
      if (storedToken == null) {
        ...
        _showResultDialog(
          context: context,
          title: 'No Stored Token',
          content: '''
No valid authentication token found.

The token is stored when you authenticate, but:
• It may have expired
• You haven't authenticated yet
• The storage was cleared

Please authenticate first to store a token.
          ''',
          isDark: isDark,
          isError: true,
        );
      ...
      // Create stateless client and fetch user info with stored token
      ...
      final userInfo = await client.getUserInfo(storedToken);

      // Also try to get extended info
      final extendedInfo = await client.getExtendedUserInfo(storedToken);

      if (!context.mounted) return;
      Navigator.pop(context); // Close loading dialog

      // Show result
      final extendedFields = extendedInfo?.availableFields.join(', ') ?? 'None';

      _showResultDialog(
        context: context,
        title: 'User Info Retrieved',
        content: '''
Successfully fetched user info with stored token:

Basic Info:
• ID: ${userInfo?.sub ?? 'N/A'}
• Name: ${userInfo?.fullName ?? 'N/A'}
• Email: ${userInfo?.email ?? 'N/A'}
• OIB: ${userInfo?.oib ?? 'N/A'}

Extended Info Fields Available:
$extendedFields

Note: The token was persisted from previous authentication.
        ''',
        isDark: isDark,
      );
